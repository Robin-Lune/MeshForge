...
- **Opt-out, anonymisation et suppression** depuis `/node/[id]` (admin). L'opt-out retire le node de tous les affichages publics individuels, dont `/nodes` et les tableaux « Liens radio » des autres fiches. L'anonymisation est **permanente** : les noms ne reviennent pas au prochain `nodeinfo` (colonne `anonymized`). La suppression efface, dans une transaction, les données alors stockées dans `nodes`, `packets`, `node_neighbors` et `traceroute_segments`, quel que soit le rôle du NodeID. Elle ne bloque pas le NodeID : un nouvel uplink MQTT le recrée normalement.
